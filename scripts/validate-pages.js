...
if (!landing.includes('href="player/"')) fail("landing page does not link to player");
if (!playerHtml.includes('src="player.js"')) fail("player script is not loaded");
...
for (const demo of player.DEMOS) {
  ...
  if (!player.hasValidGbaHeader(rom)) fail(`${relativePath} has an invalid GBA header`);
